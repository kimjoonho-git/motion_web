...
# --------------------------------------------------------------------- #
# 추가 녹화 · SELECT 가 꺼지지 않고 이어진다 · §6-86
# --------------------------------------------------------------------- #

def _overdub_status(elapsed_sec, spans=None):
    return json.dumps({
        'state': 'recording',
        'record_mode': 'overdub',
        'elapsed_sec': elapsed_sec,
        'overdub_spans': spans if spans is not None else {'1-1': [[2.36, 8.92]]},
        'execution_context': {
            'project_id': 'project-1',
            'project_generation': 4,
        },
...
def test_overdub_does_not_force_select_off_when_the_playback_ends():
    """실행 노드가 끝나는 순간 SELECT 를 모두 껐다 · 하필 사용자가 이어서
    녹화하려는 바로 그 지점이라, 다시 눌러야 했다."""
    node = playback_follow_node()
    run = {
        'state': 'running',
        'project_id': 'project-1',
    ...
    # 실제 순서 · 재생이 먼저 돌고, 스튜디오가 그걸 확인한 뒤 녹화로 넘어간다
    node._motion_run_status_callback(SimpleNamespace(data=json.dumps(run)))
    node._motion_studio_status_callback(SimpleNamespace(data=_overdub_status(3.0)))
    # 사용자가 SELECT 를 누른다 · 재생 중이라 Pickup 이 걸린 채로 대기한다
    node._control_enabled[0] = True

    # 재생이 끝난다 · 여기서 꺼지던 것이 문제였다
    run['state'] = 'completed'
    node._motion_run_status_callback(SimpleNamespace(data=json.dumps(run)))
    node._motion_studio_status_callback(SimpleNamespace(data=_overdub_status(9.2)))

    assert node._control_enabled[0] is True, '재생이 끝나자 SELECT 가 꺼졌다'


def test_the_held_line_waits_for_its_fader_so_the_motor_cannot_jump():
    """그냥 켜 두면 위험하다 · 페이더는 사용자가 놓아둔 자리에 있고 모터는
    재생을 따라 저만치 가 있다 · 구간이 끝나면 모터가 페이더 자리로 튄다.

    구간 동안 Pickup 을 걸어 두면 페이더가 모터를 따라오고 명령은 막힌다.
    """
    node = playback_follow_node()
    node._control_enabled[0] = True
    ...
    assert node._pickup.pending[0] is True, '재생 중인데 명령이 그대로 나간다'
    assert node._motor_command_state[0] == 'waiting_pickup'


def test_the_line_is_released_once_its_span_ends():
    """구간이 끝나면 더 붙잡지 않는다 · 페이더는 이미 모터 자리에 있다."""
    node = playback_follow_node()
    node._control_enabled[0] = True

    node._motion_studio_status_callback(SimpleNamespace(data=_overdub_status(3.0)))
    assert node._overdub.owned_channels([['1-1']]) == [0]

    node._motion_studio_status_callback(SimpleNamespace(data=_overdub_status(9.2)))
    assert node._overdub.owned_channels([['1-1']]) == []


def test_plain_recording_is_untouched_by_the_handoff():
    node = playback_follow_node()
    node._control_enabled[0] = True
    node._motion_studio_status_callback(SimpleNamespace(data=json.dumps({
        'state': 'recording',
        'record_mode': 'record',
        'elapsed_sec': 3.0,
        'overdub_spans': {},
        'execution_context': {'project_id': 'project-1', 'project_generation': 4},
    })))

    assert node._overdub.active is False
    assert node._pickup.pending[0] is False
    assert node._control_enabled[0] is True
