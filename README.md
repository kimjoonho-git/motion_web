...
- 처음 두 PC를 연결할 때는 양쪽 웹의 `장비 연동 상태 → 간편 PC 연동`을
  사용합니다. 중앙 PC에서 8자리 코드를 만든 뒤 참여 PC에 중앙 PC IP와 코드를
  입력하면 고유 PC ID, peer URL, 허용 IP와 PC별 HMAC 키가 자동 저장됩니다.
- 연동 코드는 5분 동안 한 번만 사용할 수 있고 코드 자체는 네트워크로 보내지
  않습니다. 처음 연결한 뒤에는 같은 설정을 다시 입력할 필요가 없습니다.
- 최초 연동 후 PC ID는 고정됩니다. 같은 PC의 IP가 바뀌면 같은 PC ID로 다시
  연동하며 기존 peer URL·암호화 키·정확한 허용 IP를 교체합니다.
- 양쪽 설정은 잠금과 중단 복구 기록을 사용해 저장합니다. 서비스가 설치되지
  않았거나 재시작되지 않으면 화면에 `부분 완료`와 해당 PC의 원인을 표시합니다.
- 자동 연동 설정은 안전한 `상태 공유`로 시작합니다. 실제 연동 실행 전에는 각
  PC에서 `연동 참여` 모드와 중앙·참여 역할을 확인해야 합니다.
- 동기 실행을 사용할 PC는 chrony 시간 동기화와 내부망 고정 IP 또는 DHCP 예약을
  사용해야 합니다. PC IP가 바뀌면 간편 연동을 다시 실행합니다.
