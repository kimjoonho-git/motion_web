...
- `filter_level`은 내부 데이터에서 0.0~1.0, 웹에서 0~100% 범위로 표시
...
0%에서는 필터를 우회한다. 나머지 설정의 이상적인 계단 응답 기준은 다음과 같다. 실제 체감은 MIDI 입력 주기와 손 움직임에 따라 달라질 수 있다.

| 필터 | 시정수 tau | 약 63% 도달 | 약 90% 도달 | 특성 |
|---:|---:|---:|---:|---|
| 0% | 0 s | 즉시 | 즉시 | 필터 없음 |
| 20% | 0.1 s | 0.21 s | 0.39 s | 약한 노이즈 완화, 빠른 반응 |
| 40% | 0.2 s | 0.43 s | 0.78 s | 비교적 부드러운 반응 |
| 60% | 0.3 s | 0.64 s | 1.17 s | 지연과 부드러움이 뚜렷함 |
| 80% | 0.4 s | 0.86 s | 1.56 s | 강한 필터, 느린 추종 |
| 100% | 0.5 s | 1.07 s | 1.95 s | 가장 부드럽고 가장 느림 |
