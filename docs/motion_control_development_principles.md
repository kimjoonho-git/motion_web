...
- `filter_level`은 0~13 정수 14단계로 관리하고 웹에서 선택 박스로 표시
- MIDI 14-bit 실시간 값, 터치, 버튼, 다이얼 상태는 뱅크에 저장하지 않음
- 뱅크 추가, 선택, 수정, 삭제 지원
- JSON/YAML 파일 읽기·쓰기 없음
- 노드 재시작 시 기본 `Bank 1`로 초기화

향후 데이터 구조가 확정되면 뱅크 설정을 모션축 설정 파일의 하위 항목으로 이동한다.

MIDI 입력 필터는 축별 2차 저역통과 필터로 고정한다. 1차 필터를 두 단계 직렬로 적용하며 수식은 다음과 같다.

```text
S = filter_level (0~13 정수)
L = S / 13
tau = L * 0.5 sec
alpha = 1 - exp(-dt / tau)
y1 = y1_prev + alpha * (x - y1_prev)
y2 = y2_prev + alpha * (y1 - y2_prev)
output = y2
```

0단계에서는 필터를 우회한다. 나머지 설정의 이상적인 계단 응답 기준은 다음과 같다. 실제 체감은 MIDI 입력 주기와 손 움직임에 따라 달라질 수 있다.

| 단계 | 시정수 tau | 약 63% 도달 | 약 90% 도달 |
|---:|---:|---:|---:|
| 0 | 0 s | 즉시 | 즉시 |
| 1 | 0.04 s | 0.08 s | 0.15 s |
| 2 | 0.08 s | 0.17 s | 0.30 s |
| 3 | 0.12 s | 0.25 s | 0.45 s |
| 4 | 0.15 s | 0.33 s | 0.60 s |
| 5 | 0.19 s | 0.41 s | 0.75 s |
| 6 | 0.23 s | 0.50 s | 0.90 s |
| 7 | 0.27 s | 0.58 s | 1.05 s |
| 8 | 0.31 s | 0.66 s | 1.20 s |
| 9 | 0.35 s | 0.74 s | 1.35 s |
| 10 | 0.38 s | 0.83 s | 1.50 s |
| 11 | 0.42 s | 0.91 s | 1.65 s |
| 12 | 0.46 s | 0.99 s | 1.80 s |
| 13 | 0.50 s | 1.07 s | 1.95 s |
